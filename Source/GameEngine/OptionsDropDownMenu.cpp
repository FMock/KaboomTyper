...
void OptionsDropDownMenu::InitializeMenuEntries()
{
    ...
    AddMenuItem("Word Speed", std::make_unique<MenuItem>(), [this](const std::string& menuItemName)
    {
...
void OptionsDropDownMenu::InitializeMenuEntry(MenuItem* menuItem, const std::string& label, int x, int y, float scale, int color)
{
    ...
    if (menuItem)
    {
        ...
        else if (label == "Word Speed")
        {
            menuItem->Initialize("WORD SPEED", menuXPos + 20, 110, menuXPos + 6, menuYPos, menuWidth, 0.65f, Colors::DARK_YELLOW);
        }
        else if (label == "Audio")
        {
