...
Button::Button() : m_body(std::make_unique<RectangleDrawable>()), m_label(std::make_unique<TextString>()), m_adjustedWidth(0), m_scaler(1.0f)
{

}

Button::Button(std::string text, int x, int y, float scaler, Colors color) : m_body(std::make_unique<RectangleDrawable>()), m_adjustedWidth(0)
{
	m_scaler = scaler;
