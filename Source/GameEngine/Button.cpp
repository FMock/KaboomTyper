...
Button::Button() : m_outline(std::make_unique<RectangleDrawable>()), m_label(std::make_unique<TextString>()), m_adjustedWidth(0), m_scaler(1.0f)
{
...
void Button::Initialize(std::string text, int x, int y, float scaler, Colors color)
{
	...
	m_outline->Initialize(x - 4, y - 4, m_adjustedWidth + 8, 32 * m_scaler + 8, Colors::DEFAULT_COLOR);
